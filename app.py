...
if uploaded_file is not None:
    ...
    st.sidebar.success(
        f"{len(MEMBER_IDS)} members loaded."
    )
...
with tab1:
    ...
    if assign_pass == PASSWORD_ASSIGN:
        ...
        if len(MEMBER_IDS) == 0:
            st.warning(
                "Please upload member CSV first."
            )
        else:
            ...
            if (
                ...
                if st.button("Generate Numbers"):
                    ...
                    st.success(
                        "Lottery numbers generated successfully!"
                    )
